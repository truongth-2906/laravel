...
@section('content')
    <div class="wrapper">
        <form id="form-create-job" autocomplete="off" action="{{ route('admin.job.store') }}"
            ...
            <div class="border-bottom pb-20 d-flex justify-content-start flex-wrap">
                ...
                <div class="form-content form-group-photo d-flex justify-content-between flex-wrap">
                    <div class="photo d-flex justify-content-center align-items-center icon-company">
                        <img src="{{ asset('img/logo-default.svg') }}" alt="avatar">
                    </div>
                </div>
            </div>
            ...
            <div class="border-bottom pb-20 d-flex justify-content-start flex-wrap">
                ...
                <div class="form-content text-color font-size-16">
                    <button type="button" class="btn btn-secondary btn-toggle btn-status-job {{ old('status') == 1 ? 'active' : '' }}" data-toggle="button"
                ...
                <div class="form-content text-color font-size-16 form-select2-container form-select-timezone-container">
                    <select id="form-select-timezone" class="js-states form-control" name="timezone_id">
                        ...
                        @foreach(getListTimezone() as $timezone)
                            <option value="{{ $timezone->id }}" data-path="{{ asset('img/clock-regular.svg') }}" {{ $timezone->id == old('timezone_id') ? 'selected' : '' }}>{{ $timezone->name }}</option>
                        @endforeach
                    </select>
                    @error('timezone_id')
                    <div class="text-danger">{{ $message }}</div>
            ...
            <div class="border-bottom pb-20 d-flex justify-content-start flex-wrap">
                ...
                <div class="form-content form-group-photo d-flex flex-column">
                    <textarea name="description" class="full-width p-2" id="job-description" rows="10" maxlength="{{ $maxDescription }}"
                              cols="80">{{ old('description') }}</textarea>

                    <div class="font-weight-400 font-size-14 description-color">
                        <span class="count-character">{{ old('description') ? getLengthBio($maxDescription, strlen(old('description'))) : $maxDescription }}</span>
            ...
            <div class="border-bottom pb-20 d-flex justify-content-start flex-wrap">
                <label class="font-weight-500 font-size-14 text-color form-title" for="form-input-wage">
                    @lang("Amount (USD)")
                </label>
                <div class="form-content">
                    <input id="form-input-wage" type="text" class="form-input-name text-color font-size-16 form-group-base" name="wage"
                ...
                <div
                    ...
                    <select id="form-select-rpa-experience" class="js-states form-control" name="experience_id">
                        ...
                        @foreach(getListExperience() as $experience)
                            <option {{ $experience->id == old('experience_id') ? 'selected' : '' }} value="{{ $experience->id }}">{{ $experience->name }}</option>
                        @endforeach
                    </select>
                    @error('experience_id')
                    <div class="text-danger">{{ $message }}</div>
