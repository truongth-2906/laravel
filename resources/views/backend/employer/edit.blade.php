...
@section('content')
    <div class="wrapper">
        <form class="form-freelancer" id="form-freelancer" method="post" enctype="multipart/form-data"
              action="{{ route('admin.employer.update', $employer->id) }}">
            @csrf
            <input type="text" class="d-none" name="type" value="{{ $employer->type }}">
            <input type="text" class="d-none" name="employer_id" value="{{ $employer->id }}">

            <div class="form-header d-flex justify-content-between align-items-center">
                <div class="text-header primary-color font-weight-500">@lang("EDIT EMPLOYER")</div>
            </div>
            ...
            <div
                ...
                <div class="form-group-button d-flex">
                    ...
                    <button type="button" class="button-base btn-general-action hover-button" id="button-save">
                        @lang("Save")
                    </button>
                    <button type="button" data-id="{{ $employer->id }}"
                            class="button-base btn-general-action hover-button btn-confirm-delete-employer">
                        @lang("Delete")
                    </button>
                </div>
            </div>
                    ...
                    <div class="d-flex justify-content-center align-content-center ">
                        <select name="company_id" id="form-select-company" class="js-states form-control">
                            ...
                            @foreach(getListCompany() as $company)
                                <option
                                    {{ $company->id == old('company_id', $employer->company_id) ? 'selected' : '' }} value="{{ $company->id }}">{{ $company->name }}</option>
                            @endforeach
                        </select>
            ...
            <div class="border-bottom pb-20 d-flex justify-content-start flex-wrap">
                ...
                <div class="form-content form-group-base">
                    <input type="text" name="email" id="form-input-email" value="{{ old('email', $employer->email) }}"
                           ...
                           placeholder="">
                    <input type="hidden" name="email_verified_at" value="{{ $employer->email_verified_at }}">
                    @error('email')
                    <div class="text-danger font-14">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-content text-color font-size-16 form-select2-container form-select-sector-container">
                    <select name="sector_id" id="form-select-sector" class="js-states form-control">
                        ...
                        @foreach(getListSector() as $sector)
                            <option
                                {{ $sector->id == old('sector_id', $employer->sector_id) ? 'selected' : '' }} value="{{ $sector->id }}">{{ $sector->name }}</option>
                        @endforeach
                    </select>
                    @error('sector_id')
                    <div class="text-danger font-14">{{ $message }}</div>
                ...
                <div class="form-content text-color font-size-16 form-select2-container form-select-timezone-container">
                    <select id="form-select-timezone" name="timezone_id" class="js-states form-control">
                        ...
                        @foreach(getListTimezone() as $time)
                            <option
                                {{ $time->id == old('timezone_id', $employer->timezone_id) ? 'selected' : '' }} value="{{ $time->id }}"
                                data-path="{{ asset('img/clock-regular.svg') }}">
                                {{ $time->name }}</option>
                        @endforeach
                    </select>
                    @error('timezone_id')
            ...
            <div class="border-bottom pb-20 d-flex justify-content-start flex-wrap">
                <div class="form-group-button-title form-title">
                    ...
                    the employer identity verified?")
                    </div>
                </div>
                <div class="form-content d-flex justify-content-start align-items-start">
                    <div class="form-check-box-group position-relative">
                        <input class="form-check-box d-none" type="checkbox" name="active" value="{{ IS_ACTIVE }}"
                               id="profile-checkbox" {{ old('active', $employer->active) == IS_ACTIVE ? 'checked' : '' }}>
