<div class="table-freelancers table-datas admin-freelancer-table">
    <div class="scroll-table">
        <table class="table w-100">
            ...
            <tr>
                ...
                <th class="color-475467 font-12 column-xl">@lang('Categories')</th>
                <th class="column-md"></th>
            </tr>
            </thead>
            <tbody>
            <form id="form-export-freelancers" action="{{ route('admin.freelancer.export') }}" method="GET">
                @forelse($freelancers as $freelancer)
                    <tr>
                        <td class="custom-checkbox column-sm">
                            <input type="checkbox" form="form-export-freelancers"
                                   id="check-box-10-{{ $freelancer->id }}"
                                   class="ipt-check-account" name="checkBoxIds[]" value="{{ $freelancer->id }}">
                            <label for="check-box-10-{{ $freelancer->id }}"
                                   class="d-flex align-items-center"></label>
                        </td>
                        <td class="column-lg">
                            <div class="d-flex justify-content-start align-items-center">
                                ...
                                <div class="d-flex flex-column align-items-start">
                                    <a href="{{ route('admin.freelancer.edit', ['id' => $freelancer->id]) }}">
                                        <div class="d-flex align-items-center font-14 color-000000 font-weight-bold">
                                            <span class="freelancer-info" title="{{ $freelancer->name }}">{{ $freelancer->name }}</span>
                                    ...
                                    <div class="font-14 color-000000 freelancer-info" title="{{ $freelancer->tag_line ?? '' }}">
                                        {{ $freelancer->tag_line ?? '' }}
                                    </div>
                                </div>
                            </div>
                        ...
                        <td class="column-lg">
                            <div class="d-flex justify-content-start align-items-center">
                                <input type="range" max="{{ \App\Domains\Auth\Models\User::SCORE_MAX_FREELANCER }}"
                                       value="{{ $freelancer->score  }}" class="mr-2 w-75 custom-input-range"
                                       disabled>
                                <div>{{ percentUser($freelancer->score, \App\Domains\Auth\Models\User::SCORE_MAX_FREELANCER) }}</div>
                            </div>
                        </td>
                        <td class="column-md">{{ optional($freelancer->last_login_at)->format('d M Y') }}</td>
                        <td class="column-md">
                            <div class="d-flex justify-content-start align-items-center">
                        ...
                        <td class="column-md">
                            <div class="d-flex justify-content-center align-items-center">
                                <a href="javascript:;" data-id="{{ $freelancer->id }}"
                                   class="btn btn-general d-flex justify-content-center align-items-center hover-button-list {{ $freelancer->is_hidden ? 'btn-unhidden-freelancer' : 'btn-hidden-freelancer' }}" title="{{ $freelancer->is_hidden ? 'Click to unhidden' : 'Click to hidden' }}">
                                   @if ($freelancer->is_hidden)
                                        <img src="{{ asset('/img/icon-eye-off.svg') }}" alt="" class="cursor-pointer">
                                   @else
                                        <img src="{{ asset('/img/icon-eye.svg') }}" alt="" class="cursor-pointer">
                                   @endif
                                </a>
                                <a href="javascript:;"
                                   class="btn btn-general d-flex justify-content-center align-items-center hover-button-list btn-confirm-delete-freelancer"
